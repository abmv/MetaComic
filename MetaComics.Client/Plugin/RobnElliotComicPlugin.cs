...
{
    public sealed class RobnElliotComicPlugin : IComicPlugin
    {
        //TODO: NO dates on this one

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds.feedburner.com/RobAndElliotComic");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "rae_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\RobAndElliot\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\RobAndElliot\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\RobAndElliot\\" + currentDate))
                {
        ...
        {
            return "Rob And Elliot";
        }

        public string ComicFeedURL
        {
            get { return "http://feeds.feedburner.com/RobAndElliotComic"; }
        }

        public string Name
        {
            get { return "Rob And Elliot"; }
        }
        ...
        {
            return "Rob And Elliot Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(Directory.GetCurrentDirectory() + @"\" + "Strip\\RobAndElliot"))
            {
                Directory.CreateDirectory(Directory.GetCurrentDirectory() + @"\" + "Strip");
            }
            return "RobAndElliot";
