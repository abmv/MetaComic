...
{
    public sealed class WigglesComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://mrwiggleslovesyou.com/mrwigglesfeed.rss");
            ...
            using (XmlReader reader = XmlReader.Create(responseStream))
            {
                ...
                foreach (SyndicationItem item in feed.Items)
                {
                    ...
                    x = x.Replace("Rehabilitating Mr. Wiggles #", "");
                    ...
                    DownloadImageFromFeed(x, item.Summary.Text);
                }
                reader.Close();
            }
        }
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                //TODO: If the image exists skip.
                string folder = CreateFolderStructure(datepublished) + "\\" + "wgls_" +
                                datepublished;
                if (!Directory.Exists(folder))
                {
                    ...
                    File.Copy("Category.xml",
                              CreateFolderStructure(datepublished) + "\\" + "\\" +
                              "Category.xml", true);
                }
                stream.Close();
            }
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Wiggles\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Wiggles\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Wiggles\\" + currentDate))
                {
        ...
        {
            get { return "http://mrwiggleslovesyou.com/mrwigglesfeed.rss"; }
        }
        ...
        {
            return "Wiggles Comic Strip Retriver";
        }
        ...
        private static string CreateFolderStructure(string datex)
        {
            string directoryString =
                Directory.GetCurrentDirectory() + @"\" + CreateDirStrip() + @"\" + CreateComicDir() + @"\" + datex;
            Directory.CreateDirectory(directoryString);
            return directoryString;
        }

        private static string CreateDirStrip()
        {
            if (!Directory.Exists(Directory.GetCurrentDirectory() + @"\" + "Strip"))
            {
                Directory.CreateDirectory(Directory.GetCurrentDirectory() + @"\" + "Strip");
            }
            return "Strip";
        }
        ...
        {
            if (!Directory.Exists(Directory.GetCurrentDirectory() + @"\" + "Strip\\Wiggles"))
            {
                Directory.CreateDirectory(Directory.GetCurrentDirectory() + @"\" + "Strip");
            }
            return "Wiggles";
