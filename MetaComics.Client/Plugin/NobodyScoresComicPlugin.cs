...
{
    public sealed class NobodyScoresComicPlugin : IComicPlugin
    {
        ...
        public void ReadComicStripFeed()
        {
            var request =
                (HttpWebRequest)
                WebRequest.Create(
                    "http://nobodyscores.loosenutstudio.com/index.php?category=comics&section=comics&rss=1");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "pfs_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Nobody Scores\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Nobody Scores\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Nobody Scores\\" + currentDate))
                {
        ...
        {
            return "Nobody Scores";
        }

        public string ComicFeedURL
        {
            get { return "http://nobodyscores.loosenutstudio.com/index.php?category=comics&section=comics&rss=1"; }
        }

        public string Name
        {
            get { return "Nobody Scores"; }
        }
        ...
        {
            return "Nobody Scores Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\Nobody Scores"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Nobody Scores");
            }
            return AppSettings.ComicArchiveFolder + "Nobody Scores";
        }
