...
{
    public sealed class TpasteForDnerComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.toothpastefordinner.com/rss/rss.php");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "tpfdnr_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Tooth Paste for Dinner\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Tooth Paste for Dinner\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Tooth Paste for Dinner\\" + currentDate))
                {
        ...
        {
            return "Tooth Paste for Dinner";
        }

        public string ComicFeedURL
        {
            get { return "http://www.toothpastefordinner.com/rss/rss.php"; }
        }

        public string Name
        {
            get { return "Tooth Paste for Dinner"; }
        }
        ...
        {
            return "Tooth Paste for Dinner Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + @"\" + "\\Tooth Paste for Dinner"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "");
            }
            return AppSettings.ComicArchiveFolder + "Tooth Paste for Dinner";
        }
