...
namespace Cliver
{
    ...
    public static class DateTimeRoutines
    {
        ...
        public static bool TryParseDateOrTime(string str, DateTimeFormat default_format, out DateTime date_time)
        {
            ParsedDateTime parsed_date_time;
        ...
        /// <returns>true if both date and time were found, else false</returns>
        public static bool TryParseDateTime(string str, DateTimeFormat default_format,
                                            out ParsedDateTime parsed_date_time)
        {
            if (TryParseDateOrTime(str, default_format, out parsed_date_time)
        ...
        public static bool TryParseDateOrTime(string str, DateTimeFormat default_format,
                                              out ParsedDateTime parsed_date_time)
        {
            parsed_date_time = null;
            ParsedDateTime parsed_date;
            ...
            if (!TryParseDate(str, default_format, out parsed_date))
            {
                if (!TryParseTime(str, default_format, out parsed_time, null))
                    return false;
                var date_time = new DateTime(DefaultDate.Year, DefaultDate.Month, DefaultDate.Day,
                                             parsed_time.DateTime.Hour, parsed_time.DateTime.Minute,
            ...
            else
            {
                ...
                {
                    var date_time = new DateTime(parsed_date.DateTime.Year, parsed_date.DateTime.Month,
                                                 parsed_date.DateTime.Day, 0, 0, 0);
                    parsed_date_time = new ParsedDateTime(parsed_date.IndexOfDate, parsed_date.LengthOfDate, -1, -1,
                                                          date_time);
                }
                else
                {
                    var date_time = new DateTime(parsed_date.DateTime.Year, parsed_date.DateTime.Month,
                                                 parsed_date.DateTime.Day, parsed_time.DateTime.Hour,
                                                 parsed_time.DateTime.Minute, parsed_time.DateTime.Second);
                    parsed_date_time = new ParsedDateTime(parsed_date.IndexOfDate, parsed_date.LengthOfDate,
                                                          parsed_time.IndexOfTime, parsed_time.LengthOfTime, date_time);
                }
            }
            return true;
        }
...
//look around the found date
                ...
                m = Regex.Match(str.Substring(parsed_date.IndexOfDate + parsed_date.LengthOfDate),
                                @"(?<=^\s*,?\s+|^\s*at\s*|^\s*[T\-]\s*)(?'hour'\d{1,2})\s*:\s*(?'minute'\d{2})\s*(?::\s*(?'second'\d{2}))?(?:\s*(?'ampm'AM|am|PM|pm))?(?=$|[^\d\w])",
                                RegexOptions.Compiled);
                if (!m.Success)
                    //look for [h]h:mm:ss <date>
                    m = Regex.Match(str.Substring(0, parsed_date.IndexOfDate),
                                    @"(?<=^|[^\d])(?'hour'\d{1,2})\s*:\s*(?'minute'\d{2})\s*(?::\s*(?'second'\d{2}))?(?:\s*(?'ampm'AM|am|PM|pm))?(?=$|[\s,]+)",
                                    RegexOptions.Compiled);
            }
            else //look anywere within string
                //look for [h]h:mm[:ss] [PM/AM]
                m = Regex.Match(str,
                                @"(?<=^|\s+|\s*T\s*)(?'hour'\d{1,2})\s*:\s*(?'minute'\d{2})\s*(?::\s*(?'second'\d{2}))?(?:\s*(?'ampm'AM|am|PM|pm))?(?=$|[^\d\w])",
                                RegexOptions.Compiled);
            if (m.Success)
            {
                try
        ...
        public static bool TryParseDate(string str, DateTimeFormat default_format, out ParsedDateTime parsed_date)
        {
            ...
            if (m.Success)
            {
                ...
                {
                    if (
                        !convert_to_date(int.Parse(m.Groups["year"].Value), int.Parse(m.Groups["day"].Value),
                ...
                {
                    if (
                        !convert_to_date(int.Parse(m.Groups["year"].Value), int.Parse(m.Groups["month"].Value),
        ...
        #region Nested type: ParsedDateTime
