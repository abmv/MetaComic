...
{
    public sealed class ComicsDotComRSSPlugin : IComicPlugin
    {
        public static string comicname = "ComicsDotCom";

        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create(AppSettings.ComicDotComRSSFeedURL);
            ...
            using (XmlReader reader = XmlReader.Create(responseStream))
            {
                ...
                foreach (SyndicationItem item in feed.Items)
                {
                    ...
                    string result = String.Join("-", x.Split('-').Skip(1));
                    char[] charsToTrim = {',', '.', ' '};
                    comicname = x.Split('-').Skip(0).FirstOrDefault().TrimEnd(charsToTrim);
                    DateTime parsedDate;
                    try
                    {
                        parsedDate = DateTime.Parse(result);
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\ComicsDotCom\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\ComicsDotCom\\" + comicname + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\ComicsDotCom\\" + comicname + currentDate))
                {
        ...
        {
            get { return AppSettings.ComicDotComRSSFeedURL; }
        }

        public string Name
        {
            get { return "ComicsDotCom"; }
        }
        ...
        {
            return "ComicsDotCom RSS Comic Strips Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\" + comicname))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + comicname);
            }
            return AppSettings.ComicArchiveFolder + @"\" + comicname;
        }
