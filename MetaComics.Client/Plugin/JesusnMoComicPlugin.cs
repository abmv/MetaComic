...
{
    public sealed class JesusnMoComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds2.feedburner.com/JesusAndMo");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "jnm_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Jesus And Mo\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Jesus And Mo\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Jesus And Mo\\" + currentDate))
                {
        ...
        {
            get { return "http://feeds2.feedburner.com/JesusAndMo"; }
        }

        public string Name
        {
            get { return "Jesus And Mo"; }
        }
        ...
        {
            return "Jesus And Mo Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Jesus And Mo"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Jesus And Mo");
            }
            return AppSettings.ComicArchiveFolder + "Jesus And Mo";
        }
