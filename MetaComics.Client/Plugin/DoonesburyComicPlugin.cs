...
{
    public sealed class DoonesburyComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.arcamax.com/doonesbury/channelfeed");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "dun_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Doonesbury\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Doonesbury\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Doonesbury\\" + currentDate))
                {
        ...
        {
            get { return "http://www.arcamax.com/doonesbury/channelfeed"; }
        }

        public string Name
        {
            get { return "Doonesbury"; }
        }
        ...
        {
            return "Doonesbury Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + @"\" + "\\Doonesbury"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Doonesbury");
            }
            return AppSettings.ComicArchiveFolder + "Doonesbury";
        }
