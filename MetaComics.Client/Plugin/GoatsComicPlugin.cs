...
{
    public sealed class GoatsComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.goats.com/index.xml");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "gts_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Goats\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Goats\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Goats\\" + currentDate))
                {
        ...
        {
            get { return "http://www.goats.com/index.xml"; }
        }
        ...
        {
            return "Goats Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Goats"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Goats");
            }
            return AppSettings.ComicArchiveFolder + "Goats";
        }
