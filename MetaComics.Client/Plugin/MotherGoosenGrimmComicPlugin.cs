...
{
    public sealed class MotherGoosenGrimmComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.arcamax.com/mothergooseandgrimm/channelfeed");
            ...
            using (XmlReader reader = XmlReader.Create(responseStream))
            {
                ...
                foreach (SyndicationItem item in feed.Items)
                {
                    ...
                    x = x.Replace("Mother Goose and Grimm for ", "");
                    DateTime parsedDate;
                    try
                    {
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "mgng_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Mother Goosen Grimm\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Mother Goosen Grimm\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Mother Goosen Grimm\\" + currentDate))
                {
        ...
        {
            return "Mother Goosen Grimm";
        }

        public string ComicFeedURL
        {
            get { return "http://www.arcamax.com/mothergooseandgrimm/channelfeed"; }
        }

        public string Name
        {
            get { return "Mother Goosen Grimm"; }
        }
        ...
        {
            return "Mother Goosen Grimm Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Mother Goosen Grimm"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Mother Goosen Grimm");
            }
            return AppSettings.ComicArchiveFolder + "Mother Goosen Grimm";
        }
