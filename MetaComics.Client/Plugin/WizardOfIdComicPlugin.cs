...
{
    public sealed class WizardOfIdComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.johnhartstudios.com/wizardofid/atom.xml");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "wid_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Wizard Of ID 1\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Wizard Of ID 1\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Wizard Of ID 1\\" + currentDate))
                {
        ...
        {
            return "Wizard Of ID 1";
        }

        public string ComicFeedURL
        {
            get { return "http://www.johnhartstudios.com/wizardofid/atom.xml"; }
        }

        public string Name
        {
            get { return "Wizard Of ID 1"; }
        }
        ...
        {
            return "Wizard Of ID 1 Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\Wizard Of ID 1"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Wizard Of ID 1");
            }
            return AppSettings.ComicArchiveFolder + "Wizard Of ID 1";
        }
