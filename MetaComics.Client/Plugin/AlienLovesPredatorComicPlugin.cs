...
{
    public sealed class AlienLovesPredatorComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://alienlovespredator.com/feed/");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "alp_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Alien Loves Predator\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Alien Loves Predator\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Alien Loves Predator\\" + currentDate))
                {
        ...
        {
            return "Alien Loves Predator";
        }

        public string ComicFeedURL
        {
            get { return "http://alienlovespredator.com/feed/"; }
        }

        public string Name
        {
            get { return "Alien Loves Predator"; }
        }
        ...
        {
            return "Alien Loves Predator Comic Strip Retriver";
        }
        ...
        public void DownloadImageFromFeed(string datepublished, Uri imagurl)
        {
            byte[] imageData = DownloadData(imagurl); //DownloadData function from here
            var stream = new MemoryStream(imageData);
            Image img = Image.FromStream(stream);
            string FileName = imagurl.ToString().Substring(imagurl.ToString().LastIndexOf("=") + 1) + ".gif";
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "aw_";
            if (!Directory.Exists(folder))
            {
                if (!File.Exists(folder + FileName))
                    img.Save(folder + FileName);
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Alien Loves Predator"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Alien Loves Predator");
            }
            return "Alien Loves Predator";
        }
