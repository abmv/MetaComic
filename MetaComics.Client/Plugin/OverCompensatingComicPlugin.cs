...
{
    public sealed class OverCompensatingComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.overcompensating.com/index.xml");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "ocs_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Over Compensating\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Over Compensating\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Over Compensating\\" + currentDate))
                {
        ...
        {
            return "Over Compensating";
        }

        public string ComicFeedURL
        {
            get { return "http://www.overcompensating.com/index.xml"; }
        }

        public string Name
        {
            get { return "Over Compensating"; }
        }
        ...
        {
            return "Over Compensating Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Over Compensating"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Over Compensating");
            }
            return AppSettings.ComicArchiveFolder + "Over Compensating";
        }
