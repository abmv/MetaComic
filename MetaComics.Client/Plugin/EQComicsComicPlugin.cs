...
{
    public sealed class EQComicsComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds2.feedburner.com/eqcomics");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "eq_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\EQ Comics\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\EQ Comics\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\EQ Comics\\" + currentDate))
                {
        ...
        {
            get { return "http://feeds2.feedburner.com/eqcomics"; }
        }

        public string Name
        {
            get { return "EQ Comics"; }
        }
        ...
        {
            return "EQ Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\EQ Comics"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "EQ Comics");
            }
            return AppSettings.ComicArchiveFolder + "EQ Comics";
        }
