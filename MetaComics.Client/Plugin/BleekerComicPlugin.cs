...
{
    public sealed class BleekerComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://bleekercomics.com/?feed=rss2");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "blkr_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Bleeker\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Bleeker\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Bleeker\\" + currentDate))
                {
        ...
        {
            get { return "http://bleekercomics.com/?feed=rss2"; }
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Bleeker"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Bleeker");
            }
            return AppSettings.ComicArchiveFolder + @"\" + "Bleeker";
        }
