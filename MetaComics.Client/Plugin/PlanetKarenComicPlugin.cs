...
{
    public sealed class PlanetKarenComicPlugin : IComicPlugin
    {
//http://planetkaren.bit-bucket.com/feed/

        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://planetkaren.girl-wonder.org/?view=rss");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "pk_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Planet Karen\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Planet Karen\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Planet Karen\\" + currentDate))
                {
        ...
        {
            get { return "http://planetkaren.girl-wonder.org/?view=rss"; }
        }

        public string Name
        {
            get { return "Planet Karen"; }
        }
        ...
        {
            return "Planet Karen Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Planet Karen"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Planet Karen");
            }
            return AppSettings.ComicArchiveFolder + "Planet Karen";
        }
