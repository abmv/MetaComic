...
{
    public sealed class NameRemovedComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds.feedburner.com/NameRemoved");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "nr_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Name Removed\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Name Removed\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Name Removed\\" + currentDate))
                {
        ...
        {
            get { return "http://feeds.feedburner.com/NameRemoved"; }
        }

        public string Name
        {
            get { return "Name Removed"; }
        }
        ...
        {
            return "Name Removed Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Name Removed"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Name Removed");
            }
            return AppSettings.ComicArchiveFolder + "Name Removed";
        }
