...
{
    public sealed class GunShowComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.rsspect.com/rss/gunshowcomic.xml");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "gs_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Gun Show Comic\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Gun Show Comic\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Gun Show Comic\\" + currentDate))
                {
        ...
        {
            return "Gun Show Comic";
        }

        public string ComicFeedURL
        {
            get { return "http://www.rsspect.com/rss/gunshowcomic.xml"; }
        }

        public string Name
        {
            get { return "Gun Show Comic"; }
        }
        ...
        {
            return "Gun Show Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\Gun Show Comic"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Gun Show Comic");
            }
            return AppSettings.ComicArchiveFolder + "Gun Show Comic";
        }
