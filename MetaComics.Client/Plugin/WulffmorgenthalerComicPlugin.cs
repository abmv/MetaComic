...
{
    public sealed class WulffmorgenthalerComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members
        //http://feeds.feedburner.com/wulffmorgenthaler
        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds.feedburner.com/wulffmorgenthaler");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "wfgr_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Wulffmorgenthaler\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Wulffmorgenthaler\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Wulffmorgenthaler\\" + currentDate))
                {
        ...
        {
            return "Wulffmorgenthaler";
        }

        public string ComicFeedURL
        {
            get { return "http://feeds.feedburner.com/wulffmorgenthaler"; }
        }

        public string Name
        {
            get { return "Wulffmorgenthaler"; }
        }
        ...
        {
            return "Wulffmorgenthaler Comic Strip Retriver";
        }
        ...
        private static string CreateFolderStructure(string datex)
        {
            string directoryString =
                Directory.GetCurrentDirectory() + @"\" + CreateComicDir() + @"\" + datex;
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Wulffmorgenthaler"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Wulffmorgenthaler");
            }
            return AppSettings.ComicArchiveFolder + "Wulffmorgenthaler";
        }
