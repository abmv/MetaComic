...
{
    public sealed class MultiplexComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds.feedburner.com/MultiplexComplete");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "mux_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Multiplexcomic\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Multiplexcomic\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Multiplexcomic\\" + currentDate))
                {
        ...
        {
            return "Multiplex Comic";
        }

        public string ComicFeedURL
        {
            get { return "http://feeds.feedburner.com/MultiplexComplete"; }
        }

        public string Name
        {
            get { return "Multiplex Comic"; }
        }
        ...
        {
            return "Multiplex Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(Directory.GetCurrentDirectory() + @"\" + "Strip\\Multiplexcomic"))
            {
                Directory.CreateDirectory(Directory.GetCurrentDirectory() + @"\" + "Strip");
            }
            return "Multiplexcomic";
        }
