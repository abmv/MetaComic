...
{
    public sealed class FlimsytheKittenComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://flimsythekitten.blogspot.com/feeds/posts/default");
            ...
            using (XmlReader reader = XmlReader.Create(responseStream))
            {
                ...
                {
                    string x = item.PublishDate.ToString("dd/MM/yyyy");
                    DateTime parsedDate;
                    try
                    {
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "ftk_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Flimsy the Kitten\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Flimsy the Kitten\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Flimsy the Kitten\\" + currentDate))
                {
        ...
        {
            return "Flimsy the Kitten";
        }

        public string ComicFeedURL
        {
            get { return "http://flimsythekitten.blogspot.com/feeds/posts/default"; }
        }

        public string Name
        {
            get { return "Flimsy the Kitten"; }
        }
        ...
        {
            return "Flimsy the Kitten Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Flimsy the Kitten"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Flimsy the Kitten");
            }
            return AppSettings.ComicArchiveFolder + "Flimsy the Kitten";
        }
