...
{
    public sealed class XKCDComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://xkcd.com/rss.xml");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "xkcd_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\XKCD\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\XKCD\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\XKCD\\" + currentDate))
                {
        ...
        {
            return "XKCD Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\XKCD"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "XKCD");
            }
            return AppSettings.ComicArchiveFolder + "XKCD";
        }
