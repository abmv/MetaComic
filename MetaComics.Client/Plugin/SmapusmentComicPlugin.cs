...
{
    public sealed class SmapusmentComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://spamusement.com/rss/");
                ...
                foreach (SyndicationItem item in feed.Items)
                {
                    ...
                    try
                    {
                        ...
                        foreach (SyndicationElementExtension extension in item.ElementExtensions)
                        {
                            ...
                            if (ele.Name.LocalName == "date" && ele.Name.Namespace.ToString().Contains("dc"))
                            {
                                parsedDate = DateTime.Parse(ele.Value);
                            }
                        }
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Spamusement\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Spamusement\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Spamusement\\" + currentDate))
                {
        ...
        {
            get { return "http://spamusement.com/rss/"; }
        }

        public string Name
        {
            get { return "Spamusement"; }
        }
        ...
        {
            return "Spamusement Cereal Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\Spamusement"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\"+ "Spamusement");
            }
            return AppSettings.ComicArchiveFolder + "Spamusement";
        }
