...
{
    public sealed class HaikucircusComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds2.feedburner.com/HaikuCircus");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "hc_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Haiku Circus\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Haiku Circus\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Haiku Circus\\" + currentDate))
                {
        ...
        {
            get { return "http://feeds2.feedburner.com/HaikuCircus"; }
        }

        public string Name
        {
            get { return "Haiku Circus"; }
        }
        ...
        {
            return "Haiku Circus Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Haiku Circus"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Haiku Circus");
            }
            return AppSettings.ComicArchiveFolder + "Haiku Circus";
        }
