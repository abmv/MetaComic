...
{
    public sealed class BlondieComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://www.arcamax.com/blondie/channelfeed");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "blnde_" +
                            datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Blondie\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Blondie\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Blondie\\" + currentDate))
                {
        ...
        {
            get { return "http://www.arcamax.com/blondie/channelfeed"; }
        }
        ...
        {
            return "Blondie Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder +"\\Blondie"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Blondie");
            }
            return AppSettings.ComicArchiveFolder + @"\" + "Blondie";
        }
