...
{
    public sealed class RobTheBotComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://robthebot.com/feed/");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "rtb_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Rob the Bot\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Rob the Bot\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Rob the Bot\\" + currentDate))
                {
        ...
        {
            get { return "http://robthebot.com/feed/"; }
        }

        public string Name
        {
            get { return "Rob the Bot"; }
        }
        ...
        {
            return "Rob the Bot Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\Rob the Bot"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Rob the Bot");
            }
            return AppSettings.ComicArchiveFolder + "Rob the Bot";
        }
