...
{
    public sealed class SilentKimblyComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds.feedburner.com/silentkimbly");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "sc_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Silent Kimbly\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Silent Kimbly\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Silent Kimbly\\" + currentDate))
                {
        ...
        {
            return "Silent Kimbly";
        }

        public string ComicFeedURL
        {
            get { return "http://feeds.feedburner.com/silentkimbly"; }
        }

        public string Name
        {
            get { return "Silent Kimbly"; }
        }
        ...
        {
            return "Silent Kimbly Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Silent Kimbly"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Silent Kimbly");
            }
            return AppSettings.ComicArchiveFolder + "Silent Kimbly";
        }
