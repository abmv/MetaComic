...
{
    public sealed class DrmcninjaComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://drmcninja.com/feed");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "dmn_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Dr McNinja\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Dr McNinja\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Dr McNinja\\" + currentDate))
                {
        ...
        {
            get { return "http://drmcninja.com/feed"; }
        }

        public string Name
        {
            get { return "Dr. McNinja"; }
        }
        ...
        {
            return "Dr. McNinja Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + @"\" + "\\Dr McNinja"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Dr McNinja");
            }
            return AppSettings.ComicArchiveFolder + "Dr McNinja";
        }
