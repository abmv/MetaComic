...
{
    public sealed class PvPComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://feeds.feedburner.com/Pvponline");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "pvp_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Player vs Player\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Player vs Player\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Player vs Player\\" + currentDate))
                {
        ...
        {
            get { return "http://feeds.feedburner.com/Pvponline"; }
        }

        public string Name
        {
            get { return "Player vs Player"; }
        }
        ...
        {
            return "PvP Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder  + "\\Player vs Player"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Player vs Player");
            }
            return AppSettings.ComicArchiveFolder + "Player vs Player";
        }
