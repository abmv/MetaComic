...
{
    public sealed class UnrehearsedRiotComicPlugin : IComicPlugin
    {
        #region IComicPlugin Members

        public void ReadComicStripFeed()
        {
            var request = (HttpWebRequest) WebRequest.Create("http://unrehearsedriot.com/feed/");
        ...
        public void DownloadImageFromFeed(string datepublished, string feeditem)
        {
            ...
            if (x.Count > 0)
            {
                ...
                string folder = CreateFolderStructure(datepublished.Remove(datepublished.Length - 1, 1)) + "\\" + "urr_" +
                                datepublished;
        ...
        public Image ComicOfTheDay()
        {
            ...
            if (Directory.Exists(Application.StartupPath + "\\Strip\\Unrehearsed Riot\\"))
            {
                di = new DirectoryInfo(Application.StartupPath + "\\Strip\\Unrehearsed Riot\\" + currentDate);
                if (Directory.Exists(Application.StartupPath + "\\Strip\\Unrehearsed Riot\\" + currentDate))
                {
        ...
        {
            return "Unrehearsed Riot";
        }

        public string ComicFeedURL
        {
            get { return "http://unrehearsedriot.com/feed/"; }
        }

        public string Name
        {
            get { return "Unrehearsed Riot"; }
        }
        ...
        {
            return "Unrehearsed Riot Comic Strip Retriver";
        }
        ...
        {
            if (!Directory.Exists(AppSettings.ComicArchiveFolder + "\\Unrehearsed Riot"))
            {
                Directory.CreateDirectory(AppSettings.ComicArchiveFolder + @"\" + "Unrehearsed Riot");
            }
            return AppSettings.ComicArchiveFolder + "Unrehearsed Riot";
        }
